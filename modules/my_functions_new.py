...
def get_delta_theta(E, On):
    theta = np.arccos(On[2, 2])
    V_0 = 3.065
    new_theta = np.arcsin(np.sqrt((E + V_0)/E) * np.sin(theta))
    return new_theta - theta


def get_dxdydz_On(atom_id, E_ind, d_PMMA, On, z):
    ...
    if (z - d_PMMA) * (z + dz - d_PMMA) < 0:
        ...
        dxdydz = On.transpose() * np.mat([[0], [0], [1]]) * ds
        phi = np.arccos(On[0, 0])
        delta_theta = get_delta_theta(ma.E_arr[E_ind], On)
        On = get_O_matrix(phi, delta_theta, On)
...
def get_coll_data(d_PMMA, E_prev, O_prev, x, z):
    ...
    ## get (dx, dy, dz)
    dxdydz, On = get_dxdydz_On(atom_id, E_ind, d_PMMA, On, z)
