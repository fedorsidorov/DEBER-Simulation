...
#    plt.loglog(raw_arr[:, 0]*1e+6, raw_arr[:, 1]*1e-24, 'ro')
#    plt.loglog(E_arr_x, result)
#    plt.show()
    
    return result

#a = get_elastic_total_cs('Si')
...
#    plt.loglog(raw_arr[:, 0]*1e+6, raw_arr[:, 1]*1e+6, 'ro')
#    plt.loglog(E_arr_x, result)
#    plt.show()
    
    return result

#a = get_excitation_dE('O')
...
def get_ionization_2nd_spectra(el_name, ss_name, ion_E2nd):
    ...
    #for i in range(len(E_unq)):
    #    inds = np.where(file_0[:, 0] == E_unq[i])
    #    e = file_0[inds, 1].transpose()[:, 0]
    #    p = file_0[inds, 2].transpose()[:, 0]
    #    plt.loglog(e, p, '-o', label=str(E_unq[i]) + ' eV')
    #plt.legend()
    #plt.axis('on')
    #plt.grid('on')
    #plt.show()
    ...
    # Нарисуем то, что отфитировали
    #for i in range(np.shape(E_unq)[0]):
    #    inds = np.where(file_0[:, 0] == E_unq[i])
    #    e = file_0[inds, 1].transpose()[:, 0]
    #    p = file_0[inds, 2].transpose()[:, 0]
    #    plt.loglog(e, p, '-o', label=str(E_unq[i]) + ' eV')
    #    plt.loglog(E_ext, E_ext_y)
    #plt.legend()
    #plt.axis('on')
    #plt.grid('on')
    #plt.show()
    ...
    #for i in range(np.shape(E_unq)[0]):
    #    inds = np.where(file_1[:, 0] == E_unq[i])
    #    e = file_1[inds, 1].transpose()[:, 0]
    #    p = file_1[inds, 2].transpose()[:, 0]
    #    plt.loglog(e, p, '-o', label=str(E_unq[i]) + ' eV')
    ##    plt.loglog(E_ext, E_ext_y)
    #plt.legend()
    #plt.axis('on')
...
#        e = file_1_x[inds, 1].transpose()[:, 0]
#        p = file_1_x[inds, 2].transpose()[:, 0]
#        plt.loglog(e, p, '-o', label=str(E_unq_x[i]) + ' eV')
#        plt.loglog(E_ext, E_ext_y)
#    plt.legend()
#    plt.axis('on')
#    plt.grid('on')
...
#    plt.show()
    ...
    # Check double interpolation
    #for i in range(0, Es, 100):
    #    print(i)
    #    plt.loglog(E_ext, spectra_1[i, :], label=str(E_arr_x[i]))
    #plt.ylim(1e-22, 1e+2)
    #plt.legend()
    #plt.axis()
    #plt.grid()
    ...
    #% Check sums
#    sums = np.zeros(np.shape(E_arr_x))
#    
#    for i in range(Es):
#        sums[i] = np.trapz(spectra_1[i, :], x=E_ext)
#    
#    plt.loglog(E_arr_x, sums)
#    plt.axis()
#    plt.grid()
#    plt.show()
    ...
    # Check cut data
    #for i in range(0, Es, 100):
    #    plt.loglog(E_ext, spectra_2[i, :], label=str(E_arr_x[i]))
    ##plt.ylim(1e-22, 1e+2)
    #plt.legend()
    #plt.axis()
    #plt.grid()
    ...
    # Check means
#    means = np.zeros(np.shape(E_arr_x))
#    
#    for i in range(Es):
#        means[i] = np.trapz(spectra_2[i, :]*E_ext, x=E_ext)
    
#    plt.loglog(E_arr_x, means, 'ro')
#    plt.loglog(E_arr_x, ion_E2nd_files)
#    plt.ylim(1e-2, 1e+4)
#    plt.axis()
#    plt.grid()
#    plt.show()
...
#    sums_new = np.zeros(np.shape(E_arr_x))
#    
#    for i in range(Es):
#        sums_new[i] = np.trapz(spectra_2[i, :], x=E_ext)
#    
#    plt.loglog(E_arr_x, sums_new)
#    plt.axis()
#    plt.grid()
#    plt.show()
    ...
    #
    for i in range(0, Es, 100):
        plt.semilogx(E_ext, ion_spectra_int_norm[i, :], label=str(E_arr_x[i]))
