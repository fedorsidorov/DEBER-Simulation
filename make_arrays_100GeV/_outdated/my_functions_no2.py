...
def get_coll_data(d_PMMA, E_prev, O_prev, x, z):
    ...
    if coll_id == 0:
        ...
        if E2nd > E_prev:
            E2nd = ma.ION_E_2ND[atom_id][E_ind, subshell_id]
...
#    sim_data = np.zeros((int(2e+4), 9))*np.nan
    sim_data = np.zeros((int(1e+5), 9))*np.nan
    pos = 0
...
def get_DATA(E0, beam_pos, D, d_PMMA, n_tracks):
...
#%% Plot DATA
def plot_DATA(DATA, track=-1):
    
    fig, ax = plt.subplots()
    
    if track >= 0:
    
        beg = np.where(DATA[:, 0] == track)[0][0]
        end = np.where(DATA[:, 0] == track)[0][-1] + 1
        ax.plot(DATA[beg:end, 5], DATA[beg:end, 7], linewidth=0.7)
        
    else:
        
        for tn in range(int(np.max(DATA[:, 0])) + 1):
