...
dest_dir = 'Wall/CUBES_C_exc_2000/'
...
for fname in fname_list[:n_files]:
...
#    for i in range(5):
    for i in range(1):
        ...
        statements = (np.all(now_DATA_Pn[:, (mv.e_x, mv.e_y, mv.e_z)] >=\
                                         mv.xyz_min, axis=1),
                      np.all(now_DATA_Pn[:, (mv.e_x, mv.e_y, mv.e_z)] <=\
                                         mv.xyz_max_new - mv.eps, axis=1))
...
#        continue
        ...
        for line in now_DATA_Pn_cut:
            ...
            xyz_cube = np.floor(xyz_coords / mv.cube_size).astype(int)
            
            ## get cube position in all_cubes_arr
            list_pos = np.dot(xyz_cube, (1, mv.n_X, mv.n_X * mv.n_Y)).astype(int)
            
            ## get cell coordinates in cube
            xyz_cell = np.floor((xyz_coords - xyz_cube * mv.cube_size) /\
                                mv.cell_size).astype(int)
