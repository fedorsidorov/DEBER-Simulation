...
## INITIAL
mat_A = np.load('mat_A_diff_Harris.npy')
...
## FINAL
mat_B = np.load('mat_B_diff_Harris.npy')

plt.plot(mat_B[:, 0], mat_B[:, 1] * 0.7, label='final, GPC')

L_2C_ion = np.load('L_final_2C_all_Harris.npy')

plt.hist(np.log10(L_2C_ion * 100), bins=20, normed=True, label='final, SIM',\
         rwidth=0.8, alpha=0.5)

plt.title('ION + EXC on 2 C atoms of 5, G(s) = 1.42')
plt.xlabel('log(m$_w$)')
